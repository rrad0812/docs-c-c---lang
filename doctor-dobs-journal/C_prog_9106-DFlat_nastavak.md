...
Aplikacioni program D-Flat biće podložan nekim konfiguracionim stavkama koje kontroliše korisnik. Korisnik će možda moći da odredi boje, opcije uređivača i tako daje. D-Flat biblioteka ukjučuje funkcije i strukture podataka koje podržavaju održavaje konfiguracije. Listing 1, stranica 148, je config.h, datoteka zaglavja koja opisuje strukture podataka o konfiguraciji. Prvi unos je #define za globalni simbol DFLAT_APPLICATION. Ovaj string imenuje vaš aplikacijski program i takođe će biti ime konfiguracione datoteke. Kao što je prikazano u Listingu jedan, string se inicijalizuje na literal stringa "memopad". Konfiguracioni fajl će, prema tome, biti MEMOPAD.CFG.
...
Listing dva, strana 148 je config.c, koji sadrži podrazumevane inicijalizovane vrednosti za konfiguracioni zapis i funkcije za čitanje i pisanje konfiguracione datoteke. Postoje dva niza boja. Jedan je za sisteme u boji, a drugi za monohromatske sisteme. Da biste promenili podrazumevane vrednosti, promenili biste ove nizove. Nizovi se sami inicijalizuju korišćenjem vrednosti boja iz datoteke zaglavlja Turbo C conio.h. Datoteka zaglavlja dflat.h, objavljena prošlog meseca, sadrži iste vrednosti za Microsoft C korisnike.
...
D-Flat radi sa prozorima u hijerarhiji klasa prozora. Ovaj model je sličan onom koji koristi Microsoft Vindovs platforma za programiranje. Kada kreirate prozor, određujete njegovu klasu i D-Flat stoga zna kako da se nosi sa prozorom. Klasa identifikuje boje prozora i određene karakteristike obrade. Prozor okvira za uređivanje se ponaša drugačije od prozora okvira sa listom, na primer. Listing tri, strana 148, je classdef.h, datoteka zaglavlja koja definiše tip podataka CLASSDEFS, koji sadrži opis klase prozora. Classdefs niz CLASSDEFS tipova podataka sadrži unos za svaku klasu. Svaki unos navodi klasu prozora koju opisuje, osnovnu klasu – ako postoji – iz koje je izvedena, boje klase prozora, adresu funkcije za obradu prozora na koju se šalju sve poruke za prozor i vrednost atributa prozora.

Vrednosti klase prozora su definisane u datoteci zaglavlja dflat.h, koju sam objavio prošlog meseca. Ako želite da dodate klase prozora u hijerarhiju, dodajte unos u enum vindov_class u dflat.h i unos u classdefs niz na Listingu četiri, stranica 148, classdefs.c. Ova tehnika se razlikuje od načina na koji to rade Vindovs programeri. Oni stavljaju runtime kod u svoje programe da registruju nove klase. D-Flat programer koristi C kompajler da registruje klase tako što kodira nove klase u tabele klasa.
...
Kompletan paket izvornog koda za D-Flat nalazi se na CompuServe-u u biblioteci 0 DDJ Foruma i na TelePath-u. Njegovo ime je D-FLAT.ARC. To je preliminarna verzija, ali ona radi. Zameniću ovu datoteku tokom meseci kako se kod menja. Trenutno se sve kompajlira i radi sa Turbo C 2.0 i Microsoft C 6.0. Postoji makefile koji uslužni programi oba kompajlera prihvataju, a postoji i jedan primer programa, program MEMOPAD, sa kojim pišem neke od ovih kolona. Ako želite da razgovarate o D-Flat-u sa mnom, moj CompuServe ID je 71101,1262, i svakodnevno pratim DDJ FORUM.

Sledećeg meseca ćemo se upoznati sa sistemom poruka D-Flatovog modela programiranja zasnovanog na događajima.

### Kutija za programerski sapun

Nedavne DDJ kolumne i članci raspravljali su o pitanju softverskih patenata. Mišljenja se razlikuju i jaka su o tome da li softverske algoritme treba zaštititi patentima. S obzirom na to da su neke sudske odluke presudile da jesu, sad se pitam koliko je takva zaštita zaista efikasna. Podseća na softverski sistem PROMIS ranih 80-ih i pravno ludilo koje ga okružuje. Nedavna kolumna sindikalnog kolumniste Džejmsa Kilpatrika podsetila je na slučaj, a izveštaj koji sledi potiče uglavnom iz njegove kolumne. Slučaj PROMIS se ne bavi specifičnim problemima patentiranja softvera, ali postavlja pitanja o sposobnosti programera da koristi bilo koju zaštitu koju zakon pruža kako bi zaštitio svoje ideje i prava. Evo šta se dogodilo.

Programer po imenu Vilijam Hamilton razvio je sistem pod nazivom PROMIS koji upravlja bazama podataka o predmetima za sprovođenje zakona. Ministarstvo pravde mu je dodelilo ugovor kako bi savezni advokati mogli da koriste PROMIS u obavljanju svojih plemenitih dužnosti. Onda, pogodi šta? Naša vlada jednostavno nije platila. Koristili su softver, a nisu ga platili. Ovo nije bio sharevare ili nešto što su preuzeli sa BBS-a. Ovo je bio ugovor i ujak Sem je odbio da poštuje njegove uslove. Nije bilo problema sa kodom. Dobro je funkcionisalo. Čuvari našeg poverenja, zaštitnici Ustava, udružena advokatska kancelarija nacije i policija okrenuli su leđa gvozdenoj obavezi prema građaninu. Održali su svoje OBEĆANjE, ali nisu održali obećanje, a Hamiltonova kompanija je otišla.

Zašto se to dogodilo? Vladin birokrata koji je bio zadužen za odobravanje plaćanja bio je bivši Hamiltonov radnik, a Hamilton ga je otpustio. Da li je zamera vladinog čoveka prema njegovom bivšem šefu uticala na njegovu odluku da zadrži plaćanje? Samo on zna.

Savezni stečajni sudija je presudio da je Ministarstvo pravde u neplaćanju i da je u stvari ukralo softver. Naredio im je da plate. Po žalbi, vlada je ponovo izgubila. Ponovo su se žalili, a Hamilton još uvek nije video novac. Dve odvojene istrage Kongresa nisu uspele da uzdrmaju stablo novca. Svaki federalni stečajni sudija kome je od tada dodeljen slučaj diskvalifikuje se iz ovog ili onog nejasnog razloga. Niko to ne želi jer prvi sudija koji je presudio protiv vlade – njegov sopstveni poslodavac – nije ponovo imenovan kada mu je istekao mandat. Da bi se povredila uvreda, federalci su prodali kopije PROMIS-a kanadskoj vladi i slobodno ih distribuirali američkoj obaveštajnoj zajednici.

S obzirom na sve ovo, koliko ćete se osećati bezbedno kada vaš brzi novi algoritam bude zaštićen verodostojnim, američkim patentom A? Ko će da se bori za tebe kada neko koristi tvoj rad? To će, naravno, zavisiti od toga koliko novca imate za advokate i ko je pirat. Takva zaštita postoji samo kada je to u interesu momaka sa velikim oružjem.

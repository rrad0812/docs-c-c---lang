...
Kompletan paket izvornog koda za D-Flat nalazi se na CompuServe-u u biblioteci 0 DDJ Foruma i na TelePath-u. Njegovo ime je DFLAT.ZIP. Zameniću ovu datoteku tokom meseci kako se kod menja. Kao što je objavljeno, sve se kompajlira i radi sa Turbo C 2.0 i Microsoft C 6.0. Postoji makefile koji uslužni programi oba kompajlera prihvataju, a postoji i jedan primer programa, program MEMOPAD, sa kojim pišem ovu kolumnu. Ako želite da razgovarate o tome sa mnom, moj CompuServe ID je 71101,1262.

### VCOMP

Često dobijam neželjene kopije malih komercijalnih uslužnih programa za koje se njihovi dobavljači nadaju da će mi se svideti i o kojima će vam reći. Jedan takav program se zove VCOMP (Vizuelno poređenje). To je program za upoređivanje tekstualnih datoteka. Do sada sam uvek koristio DIFF program koji je dolazio sa starim Aztec C kompajlerom. Nikada nisam voleo DIFF jer ne daje jasnu sliku o razlikama između fajlova. Prikazuje dva različita bloka teksta sa << i >> tokenima da kaže iz koje datoteke dolazi tekst. Ideja je da je jedan token token za izdvajanje, a drugi token za umetanje. Izvucite tekst koji se može izdvojiti i umetnuti tekst za umetanje i imate originalni fajl ili nešto slično. Nije baš intuitivno. VCOMP, s druge strane, prikazuje složenu datoteku na celom ekranu, pomerajući se na straničnim ekranom sa zajedničkim tekstom u jednoj boji i različitim blokovima teksta iz dve datoteke u dve druge boje. Postoji veliki broj operacija koje možete izvršiti pre pisanja složene datoteke, ali nisam koristio nijednu od njih. Uglavnom sam zainteresovan da vidim šta se promenilo između dve verzije datoteke C izvornog koda. Pošto sam više čovek nego dobar programer, često ubacujem gomilu novog koda u program bez testiranja od zida do zida. Nešto kasnije otkrivam da je rođena nepovezana greška. Ne sećam se kada sam poslednji put testirao deo programa koji ima grešku, i stoga se ne sećam šta sam promenio od vremena kada greška nije postojala, moram da uporedim trenutne izvorne datoteke sa starim rezervnim kopijama da vidim šta je novo. VCOMP je savršen za to. Cena je 30 dolara za DOS ili OS/2 verzije i 45 dolara za obe. Porudžbina od Vhitnei Softvare Inc., P.O. Bok 4999, Valnut Creek, CA 94596.
